...
elab "goal_size" tag:str : tactic => do
  let g ← getMainTarget
  logInfo m!"GOALSIZE {tag.getString} size={g.sizeWithoutSharing} depth={g.approxDepth}"
...
theorem trace (i : Signal defaultDomain (BitVec 8)) (t : Nat) :
  ...
  have hLt : ∀ (u : Nat), L.val u = (rd0 i u, ()) := by
    ...
    | succ m =>
      ...
      goal_size "succ: before extract_lets"
      extract_lets EXTRACTNAMES
      goal_size "succ: after extract_lets"
      ...
      goal_size "succ: before bv_decide"
  ...
  goal_size "out: before extract_lets"
  extract_lets EXTRACTNAMES_OUT
  goal_size "out: after extract_lets"
  DSLWIREEQS_T
  simp only [sigval_add, sigval_xor]
  goal_size "out: before bv_decide"
  WIREHYPS_T
