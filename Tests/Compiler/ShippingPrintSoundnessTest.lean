...
#guard Sparkle.Backend.Verilog.commentLabel "ordinary.name" == "ordinary.name"
#guard Sparkle.Backend.Verilog.commentLabel "a\nendmodule\r\nmodule extra" ==
  "a endmodule  module extra"

/- This regression checks the production comment prefix only: the raw
module identifier still needs its separate lexical contract. -/
run_cmd do
  let badName := "a\nendmodule\r\nmodule extra"
  let m := Sparkle.IR.AST.Module.empty badName
  let expected := "// Generated by Sparkle HDL\n// Module: a endmodule  module extra\n\n"
  unless (Sparkle.Backend.Verilog.toVerilog m).startsWith expected do
    throwError "source label broke the shipping line comment"
  unless (Sparkle.Backend.Verilog.toVerilog {m with isPrimitive := true}).startsWith
      "// Primitive module: a endmodule  module extra\n// This is a blackbox" do
    throwError "primitive label broke the shipping line comment"
...
run_cmd do
  ...
  for name in [``commentLabel_lineText, ``commentLabel_eq, ``renderModule_comment,
      ``normE_input_shape, ``normBody_input_shape, ``emitExpr_render,
