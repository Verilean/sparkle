...
-- out probeVal=0..7 halted=8.  Tests addIP (offADD=25): acc(r0,1,2) += base(r3,4,5).
def main : IO Unit := do
  ...
  let q := double g
  let e := add g q         -- acc(=g) + base(=q)
  -- acc = G into r0,r1,r2 ; base = Q into r3,r4,r5
  load 0 g.x; load 1 g.y; load 2 g.z
  load 3 q.x; load 4 q.y; load 5 q.z
  si 12 25; si 0 1; JIT.eval h; JIT.tick h; si 0 0     -- progStart = offADD = 25
  let mut cyc := 0; let mut halted := false
  while (!halted) && cyc < 40000 do
    JIT.eval h
