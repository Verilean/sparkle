/-
  Sim + synth test for IP.Crypto.Secp256k1FieldHW.mulHW —
  ...
  pure-data `Secp256k1Field.mul` on several operand pairs, and
...
open Sparkle.IP.Crypto.Secp256k1Field (mul p)
open Sparkle.IP.Crypto.Secp256k1FieldHW

namespace Sparkle.Tests.IP.Crypto.Secp256k1FieldHWTest
...
def main : IO Unit := do
  IO.println "=== secp256k1 bit-serial modular multiplier HW sim ==="
  (← IO.getStdout).flush
  let mut ok := true
...
end Sparkle.Tests.IP.Crypto.Secp256k1FieldHWTest
...
private def synth_secp256k1MulResult
...
#synthesizeVerilog synth_secp256k1MulResult

private def synth_secp256k1MulDone
    (start : Signal defaultDomain Bool)
    (aIn bIn : Signal defaultDomain (BitVec 256)) :
    Signal defaultDomain Bool :=
  (mulHW start aIn bIn).done

#synthesizeVerilog synth_secp256k1MulDone
