/-
  Sim + synth test for IP.Crypto.P256FieldHW.mulHW —
  ...
  pure-data `P256Field.mul` on several operand pairs, and
...
open Sparkle.IP.Crypto.P256Field (mul p)
open Sparkle.IP.Crypto.P256FieldHW

namespace Sparkle.Tests.IP.Crypto.P256FieldHWTest
...
def main : IO Unit := do
  IO.println "=== P-256 bit-serial modular multiplier HW sim ==="
  (← IO.getStdout).flush
  let mut ok := true
...
end Sparkle.Tests.IP.Crypto.P256FieldHWTest
...
private def synth_p256MulResult
...
#synthesizeVerilog synth_p256MulResult

private def synth_p256MulDone
    (start : Signal defaultDomain Bool)
    (aIn bIn : Signal defaultDomain (BitVec 256)) :
    Signal defaultDomain Bool :=
  (mulHW start aIn bIn).done

#synthesizeVerilog synth_p256MulDone
