/-
  IP.Crypto.Ed25519FieldTheorems — formal theorems about the
  pure-data field arithmetic in `IP.Crypto.Ed25519Field`.

  All theorems are pure-Nat lemmas about `(· % p)` where
  `p = 2^255 - 19`.  No `bv_decide` (the surface is Nat,
  not BitVec).  See `Tests/Verification/EquivDemo.lean` for
  the BitVec-side `#verify_eq` story.
-/
...
theorem p_pos : 0 < p := by
  unfold p
  have h : 19 < 2^255 := by decide
  omega

theorem p_ne_zero : p ≠ 0 := Nat.pos_iff_ne_zero.mp p_pos

/-! ### Range bounds — `reduce` always returns a value in `[0, p)`. -/
...
/-! ### Reduce is idempotent on values already in range. -/
...
/-! ### Addition: commutativity and associativity (mod p). -/
...
/-! ### Multiplication: commutativity, associativity, distributivity. -/
...
/-! ### sub then add round-trip (when values are in range and b ≤ a). -/
