/-
  IP.Crypto.P256FieldHW — multi-cycle modular multiplier for
  the NIST P-256 base field
  (p = 2^256 - 2^224 + 2^192 + 2^96 - 1), Signal DSL.
  ...
  exactly `(a·b) mod p`, matching `P256Field.mul`.
...
/-- P-256 prime as a 258-bit constant (headroom for the
    2·acc + a intermediate, which stays below 3p < 2^258). -/
def pBv : BitVec 258 := BitVec.ofNat 258 Sparkle.IP.Crypto.P256Field.p
...
/-- Bit-serial P-256 field modular multiplier. -/
...
end Sparkle.IP.Crypto.P256FieldHW
