/-
  IP.Crypto.Secp256k1FieldHW — multi-cycle modular multiplier
  for the secp256k1 base field (p = 2^256 - 2^32 - 977),
  Signal DSL.

  Algorithm: bit-serial "double-and-add" modular multiply
  (Russian-peasant mod p), processing the multiplier `b`
  ...
  exactly `(a·b) mod p`, matching `Secp256k1Field.mul`.
...
namespace Sparkle.IP.Crypto.Secp256k1FieldHW

open Sparkle.Core.Domain
open Sparkle.Core.Signal

/-- secp256k1 prime as a 258-bit constant (headroom for the
    2·acc + a intermediate, which stays below 3p < 2^258). -/
def pBv : BitVec 258 := BitVec.ofNat 258 Sparkle.IP.Crypto.Secp256k1Field.p
...
/-- Bit-serial secp256k1 field modular multiplier. -/
...
end Sparkle.IP.Crypto.Secp256k1FieldHW
