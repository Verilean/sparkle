...
## Headline: Sparkle JIT Exceeds Verilator

Sparkle's JIT simulation now **exceeds Verilator** on both small and large SoCs:

| SoC | Sparkle JIT | Verilator | Ratio |
|-----|------------|-----------|-------|
| **RV32I SoC** (Sparkle-native) | **14.0M cyc/s** | 8.76M | **1.60x** |
| **LiteX PicoRV32 SoC** (1730 lines) | **11.5M cyc/s** | 10.6M | **1.08x** |
...
| 3.5 | **Type-Safe JIT Wrappers** | `SimInput`/`SimOutput`/`Simulator` generated by `verilog!` macro, compile-time port safety | **Done** |
| 3.6 | **Signal Operator Refactoring** | Mixed Signal/BitVec operators (`a + 1#8`, `1#64 <<< b`), compiler fix for inline expansion | **Done** |
| 3.7 | **Imperative `<~` Assignment** | `Signal.circuit` macro with `<~` register assignment, unified loop memoization | **Done** |
| 3.8 | **AXI4-Lite Bus Protocol** | Verified AXI4-Lite slave/master IP, 14 proofs (deadlock-free, valid persistence) | **Done** |
...
| 5 | **JIT Optimization: Exceeds Verilator** | LiteX SoC 11.5M cyc/s (1.08x Verilator). 7 optimization phases, const prop, decoder guard, wire localization | **Done** |
| 5.1 | **Proof-Driven Time Skip (Temporal Oracle)** | Prove SoC idle states → skip thousands of cycles at once. Target: 100M+ effective cyc/s | Next |
| 5.2 | **RTL Reverse Synthesis** | Auto-extract Lean specifications from parsed Verilog for proof-driven optimization | Next |
...
### JIT Optimization: Exceeds Verilator (Phase 52) — DONE

Sparkle JIT now exceeds Verilator on the LiteX PicoRV32 SoC through 7 optimization phases:

1. **Dead code removal + hex masks** — eliminate `if ((uint8_t)0U)`, replace `((1ULL << w) - 1)` with hex constants
2. **Deep MUX → if-else** — convert ≥16-arm MUX chains to if-else for CPU state machines
3. **Constant/alias propagation** — new IR Phase 0, eliminates 140+ redundant wire assignments (−20% code)
4. **Self-ref register if-else** — `mux(en, val, self)` → `if (en) next = val;` for 156 registers
5. **Peripheral-skip + PCPI guard** — skip peripheral eval when boundary signals unchanged
6. **Decoder trigger guard** — skip ~174 instruction decoder lines during non-FETCH cycles, with lookahead merge (35→14 guards)
7. **evalTick wire localization** — ~270 wires moved from class members to stack-local variables (+38% single-thread)

**Remaining improvement opportunities** (documented for future work):
- Conditional tick copy for self-ref registers (+5-14%, risk of branch cost)
- CSR bus `sel=0` guard (tested: GCC CMOV already handles this)
- `_next` variable elimination (+3-5%)
- Verilator-style `__Vdly__` deferred writes (+5-10%, large refactor)
...
102 formally proved theorems across 4 files. **Found real bug**: MSTATUS WPRI fields are writable via CSRRS/CSRRW (should be read-only per RISC-V spec).

| File | Theorems | Highlights |
|------|----------|------------|
| `RV32Props.lean` | 38 | Opcode/ALUOp roundtrip, R-type field extraction, I/S/B/U/J immediate roundtrip |
| `PipelineProps.lean` | 26 | Forwarding correctness, load-use hazard, flush/NOP, x0 invariance, store-to-load fwd |
| `CSRProps.lean` | 21 | **MSTATUS WPRI bug proved**, trap→MRET MIE roundtrip, INT_MIN/−1, div-by-zero |
| `SignalDSLProps.lean` | 17 | **Signal DSL = pure spec** for ALU (ADD/SUB/AND/OR/XOR/PASS), branch, hazard, register |

Key innovation: `@[simp]` lemmas reduce `(signal).val t` through all Signal combinators via `rfl`, enabling proofs directly on synthesizable hardware code.

### AXI4-Lite Bus Protocol IP (Phase 48) — DONE

Formally verified AXI4-Lite slave/master with 14 proofs + 23 simulation tests.

- **Props**: mutual exclusion, valid persistence (AXI spec A3.3.1), deadlock-freedom, write priority
- **Slave**: 4 registers (fsm, addr, wdata, wstrb), AW+W simultaneous acceptance
- **Master**: 5 registers with command interface, read data capture
- Both synthesize to clean Verilog via `#synthesizeVerilog`

### Imperative `<~` Register Assignment (Phase 47) — DONE

`Signal.circuit` macro with imperative-style `<~` register assignment. One macro for both synthesis and simulation.

```lean
def counter {dom : DomainConfig} : Signal dom (BitVec 8) :=
  Signal.circuit do
    let count ← Signal.reg 0#8;       -- declare register
    count <~ count + 1#8;              -- assign next-state
    return count                        -- output signal

-- Works for both:
#synthesizeVerilog counter              -- → Verilog
counter.sample 10                       -- → [0, 1, 2, ..., 9]
```

Supports multiple registers, local `let` bindings, `Signal.mux`, `hw_cond`, and all Signal operators. Desugars to `Signal.loop` + `Signal.register` + `bundleAll!` at compile time.

`Signal.loop` was unified with C FFI memoized evaluation (previously only in `loopMemo`), eliminating stack overflow for simulation. `loopMemo` kept for backward compat but delegates to `loop`.

### Signal Operator Refactoring (Phase 46) — DONE

Natural operator syntax for all Signal binary operations. Mixed Signal/BitVec operands work everywhere, including inside inlined private functions called multiple times.

#### Before → After

```lean
-- Before (verbose applicative syntax):
let sum := (· + ·) <$> a <*> b
let rounded := sarBy6 ((· + ·) <$> (s0 + d1) <*> Signal.pure 32#16)
let shifted := Signal.pure 1#64 <<< pfxShift

-- After (natural operators):
let sum := a + b
let rounded := sarBy6 ((s0 + d1) + 32#16)
let shifted := 1#64 <<< pfxShift
```

#### Compiler Fix

Root cause: the synthesis compiler's early binary operator interception called `translateExprToWire` on raw BitVec constants (`@OfNat.ofNat`), corrupting metavariable state. Second identical call would fail with "Unbound variable: self". Fixed by using `extractBitVecLiteral` for constant operands.

#### New Instances

| Operator | Signal ↔ Signal | Signal ↔ Constant | Constant ↔ Signal |
|----------|:-:|:-:|:-:|
| `+` `-` `*` | `a + b` | `a + 1#8` | `1#8 + a` |
| `&&&` `\|\|\|` `^^^` | `a &&& b` | `a &&& 0xFF#8` | `0xFF#8 &&& a` |
| `<<<` `>>>` | `a <<< b` | `a <<< 2#8` | `1#64 <<< a` |
| `++` | `a ++ b` | `a ++ 0#2` | `0#24 ++ a` |
| `===` | `a === b` | `a === 0#8` | — |

### Type-Safe JIT Simulation Wrappers (Phase 45) — DONE

`verilog!` macro generates typed `SimInput`/`SimOutput`/`Simulator` structures. Port name typos and width mismatches caught at compile time.

```lean
verilog! "module counter8_en (...) ... endmodule"
open counter8_en.Verify

-- Type-safe simulation (not raw UInt64 indices!)
let sim : Simulator := { handle }
sim.step { rst := 0, en := 1 }
let out ← sim.read
IO.println s!"count = {out.count}"  -- BitVec 8, type-safe
```
