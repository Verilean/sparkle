...
**Date**: 2026-04-09
**Branch**: feature/sim-parallel (ready to merge)

---

## Headline

Sparkle JIT exceeds Verilator on single-core and multi-core SoCs:
...
| 3.1 | **Lock-Free CDC Infrastructure** | SPSC queue, rollback, formal proofs, JIT multi-domain runner | **Done** |
| 3.2 | **Compiler Improvements** | `~~~` complement, complex lambda synthesis, `hw_let` tuple destructuring | **Done** |
| 3.3 | **SV→Sparkle RTL Transpiler** | Parse PicoRV32 Verilog → Sparkle IR → JIT. 34 tests | **Done** |
| 3.4 | **Inline Verilog Formal Verification** | `verilog!` macro, auto-assert via `bv_decide`, 6 proofs zero sorry | **Done** |
| 3.5 | **Type-Safe JIT Wrappers** | `SimInput`/`SimOutput`/`Simulator` generated by `verilog!` macro | **Done** |
| 3.6 | **Signal Operator Refactoring** | Mixed Signal/BitVec operators, compiler fix for inline expansion | **Done** |
| 3.7 | **Imperative `<~` Assignment** | `Signal.circuit` macro with `<~` register assignment | **Done** |
| 3.8 | **AXI4-Lite Bus Protocol** | Verified slave/master IP, 14 proofs (deadlock-free, valid persistence) | **Done** |
| 3.9 | **RV32I Formal Verification** | 102 theorems, MSTATUS WPRI bug, Signal DSL ↔ spec equivalence | **Done** |
| 3.10 | **Linux Boot Idle-Loop Skipping** | MIE/MTIE guard, WFI fast-path, 4 oracle accuracy tests | **Done** |
| 4 | **SV Transpiler: M-Extension** | PicoRV32 MUL/DIV/REM, MemorySSA, 34 CI-safe tests | **Done** |
| 5 | **JIT Optimization Phase 52** | 10 optimization phases, LiteX firmware execution, timer oracle | **Done** |
...
|-------|-------------|--------|
| Dead code + hex masks | `if(0)` elimination, `eq(x,0)→!(x)` | +4% |
...
- Preprocessing: named blocks, integer decls, for-loop unroll, byte-lane conversion
...
- Timer oracle: skip timer_value cycles → 49 GHz effective (9,900x)
- Verified with LiteX firmware: CSR write → timer countdown

**Test coverage**: Parser 5/5, Optimizer 10/10, RV32I Verilator ALL TESTS PASSED
