...
| register types / initial values not closed literals | **GENERATOR DEFECT; failing stage now OBSERVED (2026-09-11)** | Repro and controls are permanent: `Tests/Verification/ValueParamInitRepro.lean` (run `lake build Tests.Verification.ValueParamInitRepro`; uncomment the marked line to reproduce). **OBSERVED FACTS.** (a) The emitter is correct — synthesis succeeds and the IR carries `init=7`, pinned by a `run_cmd` in that file. (b) Controls pass: the same parameter used in the BODY (`accK9`) and a direct literal init (`litInit`) both prove. (c) With `SPARKLE_DEEP_DEBUG=1` the failing circuit logs `STAGE ok: helpers collected` and then dies BEFORE any loop node is logged — the failure is inside LOOP-NODE DISCOVERY, not later. (d) Measured directly: expressions collected inside `openLams`' `withLocalDecl` scope carry free variables OUT of it (`hasFVar = true`), and `nodeOf` then analyses them outside that scope. **INTERPRETATION (not yet proven cause):** (d) is the leading candidate for the leak. **CORRECTION:** an earlier entry claimed "fails before any definition is emitted" from its failing under `SPARKLE_DEEP_NOTHM=1`; that inference was WRONG — emission starts at ~line 1512, the NOTHM check is at ~2796, so the flag does not bound the failure. Likewise "the three delab sites are ruled out" is weaker than stated: the guards not firing is consistent with never being reached. |
