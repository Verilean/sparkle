...
| register types / initial values not closed literals | **GENERATOR DEFECT, partially diagnosed (2026-09-10/11)** | Pressing this row found a real bug, not a boundary. `Signal.reg k` with `k` a value parameter: the EMITTER is correct (the wrapper resolves it; IR carries `register _tmp_a_3 init=7`; the Verilog is fine), but `#verify_elab_deep` fails with `unknown free variable` — an internal elaboration error, NOT the designed `throwError`. Isolated against a control: the same parameter used in the BODY proves (`accK9`). DIAGNOSED so far: the loop-node walk (`findRC`) does not reach the `runCircuitH` at all for this shape, so it is not an init-reification problem. It stops at a CHAIN of traversal gaps, found one at a time by digging manually: (1) `have x := v; body` is `letFun`, not `letE` — FIXED (kept, regresses nothing); (2) the result is projected out of a tuple, and the walk descends only the applied function, never a `Prod.fst`/`snd` ARGUMENT; (3) past that it reaches `initCirc.match_1`, a match auxiliary that needs unfolding rather than syntactic descent. Fixing (2) and (3) is a rewrite of `findRC` into a proper whnf-driven search rather than three more special cases — that is the actual task, and it is bigger than it first looked. The refuse-with-a-reason half remains the more important fix either way. |
