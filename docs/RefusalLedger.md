...
| register types / initial values not closed literals | **GENERATOR DEFECT; all five hypotheses ruled out (2026-09-10/11)** | `Signal.reg k` with `k` a value parameter: the EMITTER is correct (synthesis OK, IR carries `init=7`, Verilog fine) but `#verify_elab_deep` dies with `unknown free variable '_fvar.NNNN'`, an internal error rather than the designed `throwError`. Control: the same parameter in the BODY proves (`accK9`). RULED OUT, each by measurement — do not retry these: (1) init reification (the loop walk never reaches the loop); (2) the parameter-type walk (the wrapper's signature is non-dependent); (3) `retTy` escaping its `withLocalDeclD` scope (replicated standalone: no free vars, delab fine); (4) theorem generation (fails under `SPARKLE_DEEP_NOTHM=1`, before any def is emitted); (5) **all three `delab` sites — now GUARDED with `hasFVar` checks that throw a named refusal, and NONE of them fires on the failing circuit** (guards kept: they regress nothing, 38 PROVEN lines unchanged, and they convert a future scope leak at any of those sites into a diagnosable message). So the leaking term reaches elaboration by some path other than these five. Next: bisect by `SPARKLE_DEEP_NOFIRST`/`SPARKLE_DEEP_SKIP`-style gating of the emission phases, or catch the exception around each `elabSync` group to learn which generated declaration carries it. |
