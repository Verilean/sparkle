...
| register types / initial values not closed literals | **GENERATOR DEFECT, class identified, site not yet found (2026-09-10/11)** | `Signal.reg k` with `k` a value parameter: the EMITTER is correct (synthesis OK; IR carries `init=7`; Verilog fine) but `#verify_elab_deep` dies with `unknown free variable '_fvar.NNNN'` — an internal error, not the designed `throwError`. Control: the same parameter in the BODY proves (`accK9`). RULED OUT so far, each by measurement: it is not init reification (the walk never reaches the loop); not the parameter-type walk (the wrapper's signature is non-dependent — `Signal … → Signal …`); not `retTy` escaping its `withLocalDeclD` scope (reproduced the walk standalone: `hasFVar = false`, delab fine). CONFIRMED: the failure is BEFORE any definition is emitted (fails under `SPARKLE_DEEP_NOTHM=1`) and before loop-node discovery logs anything. A raw `_fvar` in the message means some term delaborated inside a local scope is used outside it; three `delab` sites exist (lines ~1420, ~1779, ~2683) and 1779 (inside `nodeOf`, under the telescope) is the remaining suspect. Next step: instrument those three sites rather than bisect further. |
