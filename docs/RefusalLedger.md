...
| register types / initial values not closed literals | **GENERATOR DEFECT (found 2026-09-10)** | Pressing this row found a real bug, not a boundary. Isolated: a value parameter used in the BODY proves (`accK9`, the demo shape) but the SAME parameter used as a register INIT fails with `unknown free variable` — an internal elaboration error, NOT the designed `throwError`. The emitter is fine: it resolves the init to a literal before the IR (measured `register _tmp_a_3 init=7`), so the circuit is emitted correctly and only the DEEP ROUTE breaks. Two things are wrong: (a) the reifier or the loop-node discovery leaks a free variable for an init that reached the elaborator via a wrapper, and (b) whatever the cause, this should surface as a refusal with a reason rather than an internal error. Filed as TODO C item; the second half is the more important one, since an internal error is what a silent gap looks like from outside. |
