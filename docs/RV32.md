...
```
picorv32.v + picorv32_soc.v
    → [SV Parser]    Parse to SV AST (250 items, 32 always blocks)
    → [Lower]         If-Conversion to Sparkle IR (152 registers)
    → [Flatten]       Inline sub-modules with name prefixing
...
GCC-compiled bare-metal firmware runs on the transpiled PicoRV32 SoC:

```bash
# Build firmware
cd firmware
riscv32-none-elf-gcc -march=rv32i -mabi=ilp32 -O2 -nostdlib \
    -T link_unified.ld -o firmware_rv32i.elf boot_rv32i.S main_rv32i.c
riscv32-none-elf-objcopy -O binary firmware_rv32i.elf firmware_rv32i.bin
python3 bin2hex.py firmware_rv32i.bin firmware_rv32i.hex

# Run via Sparkle JIT (all 4 tests pass)
...
| **Final** | **0xCAFE0000** | **ALL PASSED** |

### Key Algorithms

- **If-Conversion (Guarded Assignments)**: Walk statement tree tracking guard conditions; emit flat priority mux chain. Eliminates dead-code paths in deeply nested `case`/`if` statements.
- **Generate Block Evaluation**: Resolve `generate if (PARAM)` using parameter defaults. PicoRV32's `TWO_CYCLE_ALU=0` selects combinational ALU.
- **Concat-LHS Bit Scatter**: Handle `{imm_j[31:20], imm_j[10:1], ...} <= $signed({bits, 1'b0})` by extracting RHS bits and placing at specified target positions.
- **Byte-Strobe Memory**: Detect `if(wstrb[N]) memory[addr][hi:lo] <= wdata[hi:lo]` and generate read-modify-write with per-byte mask.
- **$signed Sign Extension**: SHL+ASR pattern with min-32-bit types to prevent uint16_t overflow.
