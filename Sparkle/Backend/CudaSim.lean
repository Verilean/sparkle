...
/-- Generate a self-contained .cu file for a single Module.
    ...
    Compile with:
      nvcc -O3 -std=c++17 -shared -Xcompiler -fPIC -o lib<module>.so <module>.cu

    The generated file is fully self-contained — no external header include —
    because the device code is CSim's own struct + `sparkle_<cls>_eval_tick`
    emitted inline (via `funcQual = __host__ __device__`).  Layout:
      1. CUDA includes.
      2. CSim device code (struct + module functions, host+device qualified).
      3. The __global__ batch kernel (one instance per thread).
      4. The host-side extern "C" JIT API (pinned staging + set/get by port).

    `cppHeaderName` is accepted for source compatibility with #33's call
    sites (and #37's benchmark) but is now unused: nothing is `#include`d. -/
def toCudaSim (m : Module) (_cppHeaderName : String := "") : String :=
  ...
  preamble ++ "\n" ++
  emitCudaDeviceCode m ++ "\n" ++
  "// ── Batch kernel ─────────────────────────────────────────────────\n" ++
  emitCudaBatchKernel m ++ "\n" ++
  emitCudaJITHostAPI m

/-- Generate .cu for a full Design (top module only). -/
