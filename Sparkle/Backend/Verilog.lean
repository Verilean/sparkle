...
/-- A label embedded in a line comment must not contain a line terminator. -/
def commentLabel (s : String) : String :=
  if s.all (fun c => c != '\n' && c != '\r') then s
  else s.map fun c => if c == '\n' || c == '\r' then ' ' else c

def moduleComment (name : String) : String :=
  "// Generated by Sparkle HDL\n" ++ "// Module: " ++ commentLabel name ++ "\n\n"

/-- Normalize common characters in a name. This alone is NOT a complete
identifier-validity contract (leading digits and keywords remain unchanged).
...
def emitModule (m : Module) : String :=
  ...
  if m.isPrimitive then
    s!"// Primitive module: {commentLabel m.name}\n" ++
    ...
    let header := moduleComment m.name ++
