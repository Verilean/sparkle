...
def topoLevels (d : Design) : List (String × Nat) :=
  ...
  -- fuel bounds depth to d.modules.length, guaranteeing termination
  let rec computeLevel (fuel : Nat) (name : String) (visited : List String) : Nat :=
    match fuel with
    | 0 => 0
    | fuel' + 1 =>
      if visited.contains name then 0
      else
        ...
        | some (_, m) =>
          ...
          if deps.isEmpty then 0
            ...
            let childLevels := deps.map fun dep =>
              computeLevel fuel' dep (name :: visited)
            (childLevels.foldl Nat.max 0) + 1

  d.modules.map fun m => (m.name, computeLevel d.modules.length m.name [])
...
def emitCudaDesignStateStruct (d : Design) : DesignStateResult :=
  ...
  match topOpt with
  | none =>
    { structText := "// ERROR: top module '" ++ d.topModule ++ "' not found\n"
    , wireEdges  := [] }
  | some top =>
  ...
  let subStructFields := sortedInstances.map fun (modName, iName, level) =>
    ...
    | none   => "  // WARNING: module '" ++ modName ++ "' not found\n"
  ...
  let structText :=
    "// Fused design state for '" ++ d.topModule ++ "' — all modules in one allocation\n" ++
    "// Generated by Sparkle HDL CUDA backend\n" ++
    "struct " ++ structName ++ " {\n" ++
    structBody ++
    "};\n"
  ...
  let wireCopyFn :=
    devQual ++ " void " ++ topName ++ "_wire_copy(" ++ structName ++ "* s) {\n" ++
  ...
  let resetFn :=
    hostDev ++ " void " ++ topName ++ "_design_reset(" ++ structName ++ "* s) {\n" ++
    "  memset(s, 0, sizeof(" ++ structName ++ "));\n" ++
    "}\n"
...
def emitCudaFusedBatchKernel (d : Design) : String :=
  let topOpt := d.modules.find? fun m => m.name == d.topModule
  match topOpt with
  | none => "// ERROR: top module '" ++ d.topModule ++ "' not found\n"
  | some top =>
  ...
  let kernelLines : List String := [
    globalQ ++ " void " ++ topName ++ "_fused_batch_kernel(",
    s!"    {structName}* states,",
    "    unsigned int N,",
    "    unsigned int numCycles,",
    "    const void* /* input_overrides reserved */) {",
...
def emitCudaDesignJITHostAPI (d : Design) : String :=
  ...
  let kernelLaunch :=
    s!"  {topName}_fused_batch_kernel" ++
    "<<<gridSize, blockSize>>>" ++
    s!"(h->d_states, h->N, numCycles, nullptr);"

  let resetLoop :=
    "  for (unsigned i = 0; i < h->N; ++i) " ++
    topName ++ "_design_reset(h->staging + i);"
